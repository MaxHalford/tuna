...
- [Usage](#usage)
  - [Streams](#streams)
    - [Streaming from a CSV file](#streaming-from-a-csv-file)
    - [Streaming `Row`s directly](#streaming-rows-directly)
    - [Streaming from multiple sources](#streaming-from-multiple-sources)
    - [Using a custom `Stream`](#using-a-custom-stream)
  - [Aggs](#aggs)
    - [Basic statistics](#basic-aggregators)
    - [`NUnique`](#nunique)
    - [`Count`](#count)
    - [`Diff`](#diff)
    - [`Union`](#union)
    - [`GroupBy`](#groupby)
    - [`SequentialGroupBy`](#sequentialgroupby)
    - [Writing a custom `Agg`](#writing-a-custom-agg)
  - [Sinks](#sinks)
    - [`CSVSink`](#csvsink)
...
- Parallelism
- At the moment each `Agg` does it's own parsing, it would be better if this was only done once
